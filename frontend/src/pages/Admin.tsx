    ...
    <div className="max-w-7xl mx-auto py-12 px-4">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-12">
        ...
        <div className="bg-base-200 p-6 rounded-2xl border border-neutral/20 shadow-lg">
          ...
          <p className="text-4xl font-black text-error">{(products as any[]).filter(p => !p.inStock).length}</p>
        </div>
        <div className="bg-base-200 p-6 rounded-2xl border border-neutral/20 shadow-lg">
              ...
              {products.map((product) => (
                <tr key={product.id} className="hover:bg-base-300/30 transition-colors group">
                  ...
                  <td className="text-center">
                    <span className={`badge border-0 font-black uppercase text-[9px] tracking-widest ${(product as any).inStock ? 'bg-success/20 text-success' : 'bg-error/20 text-error'}`}>
                      {(product as any).inStock ? 'En Stock' : 'Agotado'}
                    </span>
                  </td>
